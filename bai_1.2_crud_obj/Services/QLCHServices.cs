...
using System.Text;
using bai_1._2_crud_obj.Repositories;
using bai_1._2_crud_obj.Models;
using System.Linq;

namespace bai_1._2_crud_obj.Services
{
    public class QLCHServices
    {
        private CHRepositories _cuaHangRepositories;
        private List<CuaHang> _lstCuaHangs;
        public QLCHServices()
        {
            _cuaHangRepositories = new CHRepositories();
            _lstCuaHangs = new List<CuaHang>();
            //FakeData();
        }

        public string GetAdd(CuaHang ch)
        {
            return _cuaHangRepositories.Add(ch);
        }
        public string GetUpdate(CuaHang ch)
        {
            return _cuaHangRepositories.Update(ch);
        }
        public string GetDelete(CuaHang ch)
        {
